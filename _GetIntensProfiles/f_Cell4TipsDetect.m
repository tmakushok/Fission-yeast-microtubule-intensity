...
%-- towards the center until finding the line with a point that is white on BigRegions
        ...
        if isempty(ElemEnds)   % If one end is not found, no point in looking for the second one
            continue
        end 
...
%----- Finding the second true cell tip
        ...
        if isempty(CellCenter)   % If the second end is not found
            continue
        end
...
%% to the detected width
        ...
        if isempty(ElemWidthEnds)   % If one end is not found, no point in looking for the second one
            continue
        end 
        if BorderFlag == 1      % The cell is at the border of the image  
