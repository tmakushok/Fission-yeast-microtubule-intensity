...
% Find minima for each column in Y (for each value of X)
[Mins(:,1), Mins(:,2)] = min(Y);
% Get rid of lines with min being 0
ind = find(Mins(:,1) == 0);
Mins(ind, :) = [];
% Count which Y line number has most occurencies in Mins (correspond to the lowest histogram) 
ThePlane = mode(Mins(:,2));
